...
scal_prod(const vector_t<T>& v, const vector_t<T>& w)
{
  double result{ 0 };
  T sum;
  // suma del resultado que es la multiplicación desde la posición 0 del vector hasta la última
...
// Producto escalar entre dos vectores con numeros racionales
double
scal_prod(const vector_t<rational_t>& v, const vector_t<rational_t>& w)
{
  double resultt{ 0 };
  // suma del resultado que es la multiplicación desde la posición 0 del vector hasta la última
  // .value() para conseguir el valor del número racional
  for (int i{ 0 }; i <= v.get_size() - 1; i++) {
    resultt = resultt + (v[i].value() * w[i].value());
  }
  return resultt;
}